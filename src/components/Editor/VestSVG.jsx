    ...
    <div className="flex flex-col gap-4">
      <div className="flex gap-8 justify-center">
        ...
        <div className="flex flex-col items-center">
          <div className="text-[11px] font-semibold text-text-muted uppercase tracking-widest mb-2">{t('vest.front')}</div>
          <div className="relative w-[150px] h-[170px]">
            <GiTShirt className="w-[150px] h-[170px] text-[#252540] drop-shadow-md" />
            {FRONT_SENSORS.map(s => <Dot key={s.id} {...s} intensity={gi(s.id)} onClick={onSensorClick} t={t} />)}
          </div>
        </div>
        {/* ESPALDA */}
        <div className="flex flex-col items-center">
          <div className="text-[11px] font-semibold text-text-muted uppercase tracking-widest mb-2">{t('vest.back')}</div>
          <div className="relative w-[150px] h-[170px] opacity-85">
            <GiTShirt className="w-[150px] h-[170px] text-[#202038] drop-shadow-md" />
            {BACK_SENSORS.map(s => <Dot key={s.id} {...s} intensity={gi(s.id)} onClick={onSensorClick} t={t} />)}
          </div>
        </div>
      </div>
    </div>
