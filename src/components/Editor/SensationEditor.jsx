    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <button className="flex items-center gap-2 px-3 py-2 bg-bg-tertiary text-text-primary border border-border rounded-lg text-sm hover:bg-bg-card transition-colors" onClick={() => onSave({ ...sensation, id: sensation.id + 1, name: `${sensation.name} (${t('editor.copySuffix')})` })}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-bg-card border border-border rounded-lg p-5">
          ...
          <div className="grid grid-cols-3 gap-4 mb-4 max-sm:grid-cols-1">
            <InputField label={t('editor.frequency')} type="number" min={1} max={100} value={sensation.frequency} onChange={v => h('frequency', v)} />
            <InputField label={t('editor.duration')} type="number" min={0} max={100} value={sensation.duration} onChange={v => h('duration', v)} />
            <InputField label={t('editor.intensity')} type="number" min={0} max={100} value={sensation.intensity} onChange={v => h('intensity', v)} />
          </div>

          <div className="grid grid-cols-3 gap-4 mb-4 max-sm:grid-cols-1">
            <InputField label={t('editor.rampUp')} type="number" min={0} max={200} value={sensation.rampUp} onChange={v => h('rampUp', v)} />
            <InputField label={t('editor.rampDown')} type="number" min={0} max={200} value={sensation.rampDown} onChange={v => h('rampDown', v)} />
            <InputField label={t('editor.exitTime')} type="number" min={0} max={10} value={sensation.exitTime} onChange={v => h('exitTime', v)} />
          </div>
...
function genText(s) {
  ...
  return `${s.id}~${s.name}~${p},${bs.join('&')}~${s.icon}~${s.group}~#`;
            ...
            <div className='flex flex-col gap-1'>
              ...
              <select
                value={sensation.icon}
                onChange={(e) => h('icon', e.target.value)}
                className='bg-bg-primary border border-border rounded-lg px-3 py-2 text-text-primary text-sm focus:outline-none focus:border-accent'
              >
                {Object.entries(ICON_CATEGORIES).map(([catId, cat]) => (
                  <optgroup key={catId} label={t(`icons.${catId}`)}>
                    {cat.icons.map((icon) => (
            ...
            <div className='flex flex-col gap-1'>
              ...
              <select
                value={sensation.group}
                onChange={(e) => h('group', e.target.value)}
                className='bg-bg-primary border border-border rounded-lg px-3 py-2 text-text-primary text-sm focus:outline-none focus:border-accent'
              >
                {GROUPS.map((g) => (
        ...
        <div className='bg-bg-card border border-border rounded-lg p-5'>
          <div className='flex justify-between items-center mb-4'>
            <h3 className='text-base font-medium'>
              {t('editor.blocks')} ({sensation.blocks.length})
            </h3>
            <button
              className='flex items-center gap-2 px-3 py-1.5 bg-bg-tertiary text-text-primary border border-border rounded-lg text-sm hover:bg-bg-card transition-colors'
              onClick={() =>
                h('blocks', [
                  ...sensation.blocks,
                  {
                    sensors: [{ id: 0, intensity: 100 }],
                    name: `Block ${sensation.blocks.length + 1}`,
                  },
                ])
              }
            >
            ...
            {sensation.blocks.map((block, i) => (
              <BlockEditor
                ...
                onRemove={() =>
                  h(
                    'blocks',
                    sensation.blocks.filter((_, j) => j !== i),
                  )
                }
              />
            ))}
      ...
      <div>
        <button
          className='flex items-center gap-2 px-3 py-2 bg-bg-tertiary text-text-primary border border-border rounded-lg text-sm hover:bg-bg-card transition-colors'
          ...
          <div className='mt-4 bg-bg-card border border-border rounded-lg p-4'>
            <div className='flex items-center justify-between mb-3'>
              <h4 className='text-sm text-text-muted'>
                {t('editor.filePreview')}
              </h4>
              <button
                className='px-2 py-1 bg-bg-tertiary text-text-primary border border-border rounded text-xs hover:bg-bg-card transition-colors'
                onClick={() =>
                  navigator.clipboard.writeText(genText(sensation))
                }
              >
                {t('editor.copy')}
              </button>
            </div>
            <pre className='bg-bg-primary border border-border rounded p-4 font-mono text-xs text-accent overflow-x-auto whitespace-nowrap'>
              {genText(sensation)}
            </pre>
          </div>
        )}
  ...
  return (
    <div className='flex flex-col gap-1'>
      ...
      <input
        ...
        className='bg-bg-primary border border-border rounded-lg px-3 py-2 text-text-primary text-sm focus:outline-none focus:border-accent w-full'
      />
    </div>
  );
}
...
function genText(s) {
  ...
  const bs = s.blocks.map((b) => {
    ...
    if (b.frequency !== undefined)
      return `${b.frequency},${b.duration},${b.intensity},${b.rampUp},${b.rampDown},${b.exitTime},${b.name || ''}|${ss}`;
    return ss;
  });
  return `${s.id}~${s.name}~${p},${bs.join('&')}~${s.icon}~${s.group}~#`;
}
