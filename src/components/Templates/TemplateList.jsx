...
export default function TemplateList({ onSelectTemplate }) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        <h2 className="text-xl font-semibold mb-1">Plantillas</h2>
        <p className="text-sm text-text-muted">Selecciona una plantilla base para crear tu sensación</p>
      </div>
          ...
          <div key={catId}>
            ...
            {expanded === catId && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-3">
                {cat.templates.map(t => <TemplateCard key={t.id} template={t} onSelect={onSelectTemplate} />)}
              </div>
            )}
          </div>
        ))}
